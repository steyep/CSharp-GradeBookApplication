...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var type = char.ToUpper(parts[2][0]) + parts[2].Substring(1);
            bool isWeighted = parts[3] == "true";
            BaseGradeBook gradeBook;
            if (type == "Standard" || type == "Ranked")
            {
                Type gradeBookType = Type.GetType("GradeBook.GradeBooks." + type + "GradeBook");
                gradeBook = (BaseGradeBook) Activator.CreateInstance(gradeBookType, name, isWeighted);
